...
public class ExpressionGenerator {
    ...
    public static void compileExpression(Expression expression, CodeEmitter emitter, VariableScope scope) {
        if (expression instanceof BinaryOperatorExpression) {
            compileBopExpression((BinaryOperatorExpression) expression, emitter, scope);
        } else if (expression instanceof BinarySelectorExpression) {
            compileBinarySelectorExpression((BinarySelectorExpression) expression, emitter, scope);
            ...
            compileFunctionCallExpression((FunctionCall) expression, emitter, scope);
        } else if (expression instanceof NegateExpression) {
            compileNegateExpression((NegateExpression) expression, emitter, scope);
        } else if (expression instanceof VariableIncrementExpression) {
            compileVariableIncrementExpression((VariableIncrementExpression) expression, emitter, scope);
        }
    }

    private static void compileBopExpression(BinaryOperatorExpression bopExpression, CodeEmitter emitter, VariableScope variableScope) {
        compileExpression(bopExpression.getLeft(), emitter, variableScope);
        compileExpression(bopExpression.getRight(), emitter, variableScope);
    ...
    private static void compileBinarySelectorExpression(BinarySelectorExpression binarySelectorExpression, CodeEmitter emitter, VariableScope scope) {
    ...
    private static void compileNegateExpression(NegateExpression negateExpression, CodeEmitter emitter, VariableScope scope) {
        ...
        compileExpression(negateExpression.getExpression(), emitter, scope);
        emitter.emitLine("sub");
    }

    private static void compileVariableIncrementExpression(VariableIncrementExpression expression, CodeEmitter emitter, VariableScope variableScope) {
    ...
    private static void compileFunctionCallExpression(FunctionCall functionCall, CodeEmitter emitter, VariableScope variableScope) {
        for (Expression expression : functionCall.getArguments()) {
            compileExpression(expression, emitter, variableScope);
        }
        emitter.emitLine("call $" + functionCall.getFunctionName());
    }
}
